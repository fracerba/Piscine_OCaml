let () =
	...
	in let print_incomplete alk n lst =
		print_string "  ";
		print_list (alk @ [(new Molecule.dioxygen, n)]);
		print_string " --> ";
		print_list lst;
		print_newline ();
	in let print_incomplete_reaction reaction alk =
		print_endline "Possible incomplete reactions:";
		List.iter (fun (n, lst) -> print_incomplete (get_sorted_list alk) n lst) (reaction#get_incomplete_results)
	in let test_reaction lst =
		...
		try
			...
			print_incomplete_reaction reaction lst;
			try
	...
	and combustion_13 = new Alkane.ethane#to_list 2 @ new Alkane.methane#to_list 1 @ new Alkane.propane#to_list 1 in
